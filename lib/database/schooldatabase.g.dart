    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Student` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `fname` TEXT, `lname` TEXT, `username` TEXT NOT NULL, `password` TEXT NOT NULL)');
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Subject` (`subId` INTEGER PRIMARY KEY AUTOINCREMENT, `subName` TEXT NOT NULL)');
...
class _$SubjectDAO extends SubjectDAO {
  ...
  @override
  Future<Subject?> findSubjectById(int subId) async {
    return _queryAdapter.query('SELECT * FROM subject WHERE subId = ?1',
